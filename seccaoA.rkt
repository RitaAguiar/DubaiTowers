...
;Cria uma unidade do corpo da secção A.

;Criação de listas de vértices para a secção A mais externa.
(define lista-base-seccaoA (map-division (cria-vertices raio-seccaoA) 0 2pi n-faces-seccaoA))
;Criação de listas de vértices para a secção A mais interna para quando da utiliação de volumes criar superfícis para extrudir.
(define lista-base-interior-seccaoA (map-division (cria-vertices (* .97 raio-seccaoA)) 0 2pi n-faces-seccaoA))


;;;;;;;CONSTRUTOR DA UNIDADE DA SECÇÃO A

(define (seccaoA cota-unid unid au transf codigo u-i u-f c-i c-f lista-dados)
  ...
  ;Cria uma das faces do corpo da secção A
  ...
  ;À excepção da primeira unidade constrói na sua base uma laje com uma abertura para a caixa do elevador
  (cond ((and (not (= unid u-i))(= codigo 2))
        (with-current-layer "Betão" (subtraction
             (extrusion (surface (closed-line (map t lista-base-seccaoA))) espessura-laje-interior)
  ...
  ;Para a primeira unidade constrói na sua base uma laje sem abertura para o elevador.
  (cond ((and (= unid u-i)(= codigo 2))     
             (with-current-layer "Betão" (extrusion (surface (closed-line (map t lista-base-seccaoA))) espessura-laje-interior))))
  ;Constrói todas as faces da secção A
  (constroi-faces lista-base-seccaoA lista-base-interior-seccaoA)
  ...
  (cond ((and (= unid u-f)(= codigo 2))
        (with-current-layer "Betão" (subtraction
             (extrusion (surface (closed-line (map (lambda (ponto) (t (+c ponto (xyz 0 0 au)))) lista-base-seccaoA))) espessura-laje-interior)
