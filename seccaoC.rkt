...
;Cria uma unidade do corpo da secção C.

;Criação de listas de vértices para a secção C mais externa.
(define lista-base-seccaoC (map-division (cria-vertices raio-seccaoC) 0 2pi n-faces-seccaoC))
;Criação de listas de vértices para a secção C mais interna para quando da utiliação de volumes criar superfícis para extrudir.
(define lista-base-interior-seccaoC (map-division (cria-vertices (* .97 raio-seccaoC)) 0 2pi n-faces-seccaoC))


;;;;;;;CONSTRUTOR DA UNIDADE DA SECÇÃO C

(define (seccaoC cota-unid unid au transf codigo u-i u-f c-i c-f lista-dados)
  ...
  ;Cria uma das faces do corpo da secção C
  ...
  (cond ((= codigo 2)    
             ...
             (extrusion (surface (closed-line (map t lista-base-seccaoC))) espessura-laje-interior)
             (move (extrusion (surface (closed-line (map t lista-base-elevador))) au) (z (- (/ au 2))))))))
  ;Constrói todas as faces da secção C
  (constroi-faces lista-base-seccaoC lista-base-interior-seccaoC)
  ...
  (cond ((and (= unid u-f)(= codigo 2))
        (with-current-layer "Betão" (subtraction
             (extrusion (surface (closed-line (map (lambda (ponto) (t (+c ponto (xyz 0 0 au)))) lista-base-seccaoC))) espessura-laje-interior)
